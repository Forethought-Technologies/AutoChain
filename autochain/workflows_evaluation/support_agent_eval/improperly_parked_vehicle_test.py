...
Output values: illegally_parked: bool, message: str""",
    ...
    test_cases = [
        TestCase(
            test_name="found name and resolved",
        ...
        TestCase(
            test_name="no id provided, hand off to agent",
            user_query="I parked a Lime vehicle on the street, can you tell me if it's "
            "properly parked?",
        ...
        TestCase(
            test_name="check status of vehicle without id",
...
if __name__ == "__main__":
    tester = WorkflowTester(
        tests=[TestImproperlyParkedVehicle()], output_dir="./test_results"
    )
    args = get_test_args()
    if args.interact:
